...
import { View, StyleSheet, ScrollView, TextInput, Image, TouchableOpacity, FlatList, Dimensions, Platform, ImageBackground } from 'react-native';
...
const VIDEO_DATA: VideoItem[] = [
  {
    id: 'v1',
    foodId: '1', // References the first food item
  ...
  {
    id: 'v2',
    foodId: '2', // References the second food item
  ...
  {
    id: 'v3',
    foodId: '3', // References the third food item
...
// We no longer need separate card components as we're using a combined approach
...
export default function HomeScreen() {
  ...
  const handleFoodItemPress = (itemId: string) => {
    router.push(`/(buyerTabs)/food-details/${itemId}` as any);
  };

  // Combine food and video data to create a single data source
  const combinedData = FOOD_ITEMS_DATA.map(foodItem => {
    // Find the corresponding video for this food item
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity 
            ...
            onPress={() => handleFoodItemPress(item.foodItem.id)}
          >
            <ImageBackground 
            ...
            >
              {/* Show food details or video based on state */}
              {!showVideos ? (
                // Food details view
                <View style={styles.foodCardOverlay}>
                  ...
                  <View style={styles.foodDetailsContainer}>
                    ...
                    <View style={styles.foodMetaContainer}>
                      <Text style={styles.foodMetaText}>Plates Left: {item.foodItem.platesLeft}</Text>
                ...
                <View style={styles.videoOverlay}>
                  <View style={styles.playButtonCircle}>
                    <FontAwesome name="play" size={20} color="#FF0000" />
                  </View>
                  
                  {/* Video title overlay at top */}
                  <View style={styles.videoTitleOverlay}>
                    <Text style={styles.videoTitleText}>30sec Commercial | Product Video</Text>
...
const styles = StyleSheet.create({
  ...
  foodName: {
    ...
    fontWeight: 'bold',
    marginBottom: 5,
    marginRight: 60, // Space for price badge
  },
  priceBadge: {
    position: 'absolute',
    top: 15,
  ...
  playButtonCircle: {
    ...
    justifyContent: 'center',
    alignItems: 'center',
    paddingLeft: 4, // Adjust for play icon centering
  },
  videoTitleOverlay: {
    position: 'absolute',
    top: 0,
